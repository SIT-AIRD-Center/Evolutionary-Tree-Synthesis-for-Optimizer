...
class Adam_Momentum(TreeOptimizer):
    def __init__(self, learning_rate = 0.001, alpha = 0.9):
        self.model_params = list()
        self.grads_params = list()
        self.tree = FuncNode(max_depth=self.max_depth, function = MulFunc())
        self.tree.args[0] = ConstNode(depth = 1, constant = learning_rate)
        self.tree.args[1] = FuncNode(max_depth = self.max_depth, function = AddFunc())
        self.tree.args[1].args[0] = FuncNode(max_depth = self.max_depth, function = MulFunc())
        self.tree.args[1].args[0].args[0] = ConstNode(depth = 3, constant = alpha)
        self.tree.args[1].args[0].args[1] = VarNode(depth = 3, variable_id = 3)
        self.tree.args[1].args[1] = FuncNode(max_depth = self.max_depth, function = MulFunc())
        self.tree.args[1].args[1].args[0] = FuncNode(max_depth = self.max_depth, function = SubFunc())
        self.tree.args[1].args[1].args[0].args[0] = ConstNode(depth = 4, constant = 1.0)
        self.tree.args[1].args[1].args[0].args[1] = ConstNode(depth = 4, constant = alpha)
        self.tree.args[1].args[1].args[1] = VarNode(depth = 3, variable_id = "gradient")
